import {
  ...
  external_exports,
  generateObject
} from "./chunk-C6IYUF6X.mjs";
import "./chunk-UYSYBDR2.mjs";
import {
  task
...
__name(normalizeRoomPermissions, "normalizeRoomPermissions");
...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("./lib-FITPWXTZ.mjs")).default;
}
...
function normalizeCreateRoomOptions(options2) {
  return {
    ...options2,
...
var NodeSchema = external_exports.object({
  id: external_exports.string().describe("Unique node ID, e.g. 'ai-node-1'"),
  label: external_exports.string().describe("Display label for the node"),
  shape: external_exports.enum(["rectangle", "diamond", "circle", "pill", "cylinder", "hexagon"]).describe("Node shape"),
  colorIndex: external_exports.number().int().min(0).max(7).describe("Color index 0-7: 0=dark, 1=blue, 2=purple, 3=orange, 4=red, 5=pink, 6=green, 7=teal"),
  x: external_exports.number().describe("X position on canvas"),
  y: external_exports.number().describe("Y position on canvas"),
  width: external_exports.number().min(80).max(400).describe("Node width in pixels"),
  height: external_exports.number().min(50).max(200).describe("Node height in pixels")
});
var EdgeSchema = external_exports.object({
  id: external_exports.string().describe("Unique edge ID, e.g. 'ai-edge-1'"),
...
var DesignSchema = external_exports.object({
  summary: external_exports.string().describe("One-sentence summary of the generated architecture"),
  nodes: external_exports.array(NodeSchema).min(3).max(14),
  edges: external_exports.array(EdgeSchema).min(1).max(20)
});
...
Generate a complete, realistic architecture with 5-12 nodes and meaningful edges showing data flow.`,
      ...
      const msg = error3 instanceof Error ? error3.message : "Unknown error";
